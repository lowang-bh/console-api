...
func (o *DeleteAppURL) Build() (*url.URL, error) {
	...
	var _path = "/groups/{group}/apps/{app}"

	app := swag.FormatInt64(o.App)
	if app != "" {
		_path = strings.Replace(_path, "{app}", app, -1)
	} else {
		return nil, errors.New("App is required on DeleteAppURL")
	}
	...
	if group != "" {
		...
		return nil, errors.New("Group is required on DeleteAppURL")
	}
...
func (o *DeleteAppURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteAppURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteAppURL")
	}
